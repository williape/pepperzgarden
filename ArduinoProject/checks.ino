...
void buttonCheck() { 
  ...
  if (digitalRead(SWITCH) == LOW && lastSwitch == HIGH) {
    delay(10); // delay to allow device to physically stabilize after button press
    Serial.println("test switch pressed");
    digitalWrite(PROBEPOWER, HIGH);
    long moistLevel = analogRead(MOISTPIN); // take a moisture reading
    digitalWrite(PROBEPOWER, LOW); 
    // assemble a string for Twitter
    char *str1 = "Current Soil Moisture: ";
    char *str2;
    str2= (char*) calloc (4,sizeof(char)); // allocate memory to string 2
    int moistPct = (moistLevel*100)/900;  // moisture is on a scale from 0 to 900. 
    ...
    Serial.println(message); // print twitter message to serial interface
    ...
    if (digitalRead(SWITCH) == LOW) { // if switch is held down, send a second tweet with the version number
      ...
      Serial.println(Ethernet.localIP()); // show local IP address
    }
  }
